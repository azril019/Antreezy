...
export default function TransactionHistoryPage() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const completedOrders = orders.filter((order: any) => {
        const hasValidStatus = order?.status && ["settlement", "capture"].includes(order.status);
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      settlement: { color: "bg-green-100 text-green-800", icon: CheckCircle, label: "Berhasil" },
      capture: { color: "bg-green-100 text-green-800", icon: CheckCircle, label: "Berhasil" },
  ...
  const exportToCSV = () => {
    ...
    const headers = ["ID Pesanan", "Meja", "Pelanggan", "Total", "Status", "Tanggal"];
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600">Kelola dan analisis riwayat transaksi restoran</p>
          </div>
          <button
            onClick={exportToCSV}
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{stats.successfulTransactions}</p>
              </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredTransactions.map((transaction) => (
                    <tr key={transaction._id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">#{(transaction.orderId || '').slice(-6).toUpperCase() || 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">Meja {transaction.tableId || 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{transaction.customerDetails?.name || 'Unknown'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{Array.isArray(transaction.items) ? transaction.items.reduce((sum, item) => sum + (Number(item.quantity) || 0), 0) : 0} item</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{formatCurrency(transaction.totalAmount)}</td>
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-semibold text-gray-900">
                  Detail Transaksi #{(selectedTransaction.orderId || '').slice(-6).toUpperCase() || 'N/A'}
                </h3>
                <button
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <p><span className="text-gray-500">ID:</span> {selectedTransaction.orderId || 'N/A'}</p>
                    <p><span className="text-gray-500">Meja:</span> {selectedTransaction.tableId || 'N/A'}</p>
                    <p><span className="text-gray-500">Status:</span> {getStatusBadge(selectedTransaction.status)}</p>
                    <p><span className="text-gray-500">Metode Bayar:</span> {selectedTransaction.paymentMethod || 'Unknown'}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <p><span className="text-gray-500">Nama:</span> {selectedTransaction.customerDetails?.name || 'Unknown'}</p>
                    {selectedTransaction.customerDetails?.email && (
                      <p><span className="text-gray-500">Email:</span> {selectedTransaction.customerDetails.email}</p>
                    )}
                    {selectedTransaction.customerDetails?.phone && (
                      <p><span className="text-gray-500">HP:</span> {selectedTransaction.customerDetails.phone}</p>
                    )}
                    <p><span className="text-gray-500">Tanggal:</span> {formatDate(selectedTransaction.createdAt)}</p>
                  </div>
                </div>
              </div>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {Array.isArray(selectedTransaction.items) && selectedTransaction.items.length > 0 ? (
                        selectedTransaction.items.map((item, index) => (
                          <tr key={index}>
                            <td className="px-4 py-2 text-sm text-gray-900">{item.name || 'Unknown Item'}</td>
                            <td className="px-4 py-2 text-sm text-gray-500">{item.quantity || 0}</td>
                            <td className="px-4 py-2 text-sm text-gray-500">{formatCurrency(item.price || 0)}</td>
                            <td className="px-4 py-2 text-sm font-medium text-gray-900">{formatCurrency((item.price || 0) * (item.quantity || 0))}</td>
                          </tr>
                        ))
                      ) : (
                        <tr>
                          <td colSpan={4} className="px-4 py-2 text-sm text-gray-500 text-center">Tidak ada item</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
