...
export const createPayment = async (paymentData: PaymentData): Promise<PaymentResult> => {
  try {
    ...
    if (!response.ok) {
      throw new Error(result.details || result.error || `HTTP ${response.status}: Failed to create payment`);
    }
...
export const initiateMidtransPayment = (token: string): Promise<any> => {
  return new Promise((resolve, reject) => {
    console.log('Initiating Midtrans payment with token:', token ? 'present' : 'missing');
